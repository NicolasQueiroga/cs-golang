...
Node *Parser::parseStatement()
{
    ...
    if (tokenizer.next.type == "voip")
    {
        ...
        if (tokenizer.next.type == "LPAREN")
        {
            ...
            {
                tokenizer.selectNext();
                if (tokenizer.next.type != "NEWLINE" && tokenizer.next.type != "COMMA")
        ...
        if (tokenizer.next.type == "IDENTIFIER")
        {
            ...
            if (tokenizer.next.type == "LPAREN")
            {
                ...
                if (tokenizer.next.type == "DECLARATION")
                {
                    ...
                    {
                        tokenizer.selectNext();
                        if (tokenizer.next.type == "NEWLINE" || tokenizer.next.type == "LBRACE")
                        {
                            tokenizer.selectNext();
                ...
                else if (tokenizer.next.type == "NEWLINE" || tokenizer.next.type == "LBRACE")
                {
                    tokenizer.selectNext();
                    std::vector<Node *> func_children;
                    while (tokenizer.next.type != "RBRACE")
    ...
    {
        children[0] = parseRelExpr();
        if (tokenizer.next.type == "NEWLINE" || tokenizer.next.type == "RBRACE")
        {
    ...
    else if (tokenizer.next.type == "if")
    {
        ...
        if (tokenizer.next.type == "LBRACE")
        {
            ...
            tokenizer.selectNext();
            while (tokenizer.next.type != "else" && tokenizer.next.type != "RBRACE")
            {
    ...
    else if (tokenizer.next.type == "round_loop")
    {
        children[0] = parseRelExpr();
        if (tokenizer.next.type == "LBRACE")
        {
    ...
    else if (tokenizer.next.type == "NEWLINE" || tokenizer.next.type == "COMMA")
        return new NoOp();
    else
...
Node *Parser::run(std::string code)
{
    ...
    tokenizer.selectNext();
    // tokenizer.selectNext();
    // std::cout << "Token type: " << tokenizer.next.type << " Value: " << tokenizer.next.value << std::endl;
    // while (tokenizer.next.type != "EOF")
    // {
    //     tokenizer.selectNext();
    //     std::cout << "Token type: " << tokenizer.next.type << " Value: " << tokenizer.next.value << std::endl;
